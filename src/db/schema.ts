...
export const orders = pgTable("orders", {
  ...
  paypal_subscription_id: varchar({ length: 255 }),
  paypal_plan_id: varchar({ length: 255 }),
  payment_provider: varchar({ length: 50 }).default("stripe"), // stripe, paypal, creem
});
...
export const ai_workstation_images = pgTable("ai_workstation_images", {
  id: integer().primaryKey().generatedAlwaysAsIdentity(),
  uuid: varchar({ length: 255 }).notNull().unique(),
  created_at: timestamp({ withTimezone: true }),
...
export const comments = pgTable("comments", {
  ...
  created_at: timestamp({ withTimezone: true }),
  updated_at: timestamp({ withTimezone: true }),
  status: varchar({ length: 50 }).notNull().default("active"), // active, hidden, deleted
  post_uuid: varchar({ length: 255 }).notNull(),
  user_uuid: varchar({ length: 255 }),
  content: text().notNull(),
  // Guest comments fields (when user is not logged in)
  guest_name: varchar({ length: 100 }),
  guest_email: varchar({ length: 255 }),
  guest_website: varchar({ length: 255 }),
  // Reply functionality
  parent_id: integer(),
  reply_to_uuid: varchar({ length: 255 }),
  // Moderation
  ip_address: varchar({ length: 45 }),
  user_agent: text(),
  is_approved: boolean().notNull().default(true),
});